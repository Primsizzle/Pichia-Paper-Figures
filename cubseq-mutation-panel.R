...
# load data
print("Loading data.")
vcf <- read.vcfR("path/to/merged/vcf", verbose = FALSE)
...
# load data
print("Loading data.")
var <- read_csv("path/to/mutation-frequency-data.csv")
gtf <- rtracklayer::import("path/to/RNASeq-genomic.gtf")
gene_tpm_scores <- read_csv("path/to/tpm-scores.csv")
...
circos.track(result$sectors, y = result$AC_norm,
             panel.fun = function(x, y) {
               ...
               circos.yaxis(side = "left", 
                            at = c(0, 0.002, 0.004, 0.006, 0.008, 0.01), # set y-axis tick values
                            labels = TRUE, 
                            labels.cex = par("cex"))
             })
